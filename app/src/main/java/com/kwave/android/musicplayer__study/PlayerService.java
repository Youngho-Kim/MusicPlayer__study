...
//        stopService(intent);
    ...
    // 1. 포어그라운드 서비스를 생성하기 위해서 노티바를 생성
    private void buildNotification(NotificationCompat.Action action, String action_flag){
        ...
        // 가.2. 서비스 졸요 intent 등록
        builder.setContentIntent(pendingDeleteIntent);

        // 가.3. 노티바에서 사용할 action(버튼) 등록

        NotificationCompat.Action prevAction = makeAction(android.R.drawable.ic_media_previous,"Prev",Const.Action.PLAY);
        builder.addAction(prevAction);
//        NotificationCompat.Action playAction = makeAction(android.R.drawable.ic_media_play,"Prev",Const.Action.PLAY));
...
//        manager.notify(NOTIFICATION_ID, builder.build());
        // 나.3 foreground로 서비스 등록하기
        startForeground(NOTIFICATION_ID, builder.build());

    }


    // 2. 노티바에서 사용하는 버튼을 생성하는 함수
    private NotificationCompat.Action makeAction(int btnIcon, String btnTitle, String action){
        Intent intent = new Intent(getApplicationContext(), PlayerService.class);
        intent.setAction(action);

        // PendingIntent : 인텐트를 서비스 밖 혹은 액티비티 밖에서 실행할 수 있도록 담아두는 주머니
